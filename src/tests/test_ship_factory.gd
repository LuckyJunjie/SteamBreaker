...
func test_spawn_ship_at_sets_position():
    var hull_res = load("res://src/resources/ships/SteamBreaker_Hull.tres")
    if hull_res == null:
        skip("Hull resource not available, skipping position test")
        return
    
    var test_pos = Vector2(500, 300)
    var ship: Node2D = _ship_factory.SpawnShipAt("res://src/resources/ships/SteamBreaker_Hull.tres", test_pos)
...
func test_get_default_hull_returns_resource():
    ...
    assert_eq(hull.resource_path, "res://resources/ships/SteamBreaker_Hull.tres",
        "ISSUE: GetDefaultHull returns res://resources/ships/ (non-src) but actual file is in res://src/resources/ships/")
...
func test_ship_loadout_weight_calculation():
    ...
    # 加载部件资源
    var hull = load("res://src/resources/parts/hull_scout.tres")
    var boiler = load("res://src/resources/parts/boiler_single_expansion.tres")
    var helm = load("res://src/resources/parts/helm_manual.tres")
...
func test_ship_loadout_max_hp_from_hull():
    ...
    var hull = load("res://src/resources/parts/hull_ironclad.tres")
    if hull:
        loadout.hull = hull
...
func test_ship_factory_path_inconsistency_bug():
    ...
    if from_wrong_path == null and from_correct_path != null:
        assert_true(true,
            "BUG CONFIRMED: ShipFactory uses wrong path 'res://resources/ships/' but actual is 'res://src/resources/ships/'")
